...
<div class="p-6">
    <div class="flex items-center gap-4 mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-900 flex-1">Edit Foto Galeri</h1>
    </div>
    ...
    <form action="{{ route('admin.gallery.update', $gallery) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl shadow-2xl p-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid md:grid-cols-2 gap-8">
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-3">
                    <i class="fas fa-image mr-2 text-blue-600"></i>Gambar Saat Ini
                </label>
                <div class="relative">
                    <img src="{{ Storage::url($gallery->image) }}" 
                         alt="{{ $gallery->title }}"
                         class="w-full h-80 object-cover rounded-2xl shadow-2xl">
                    <div class="absolute top-4 right-4 bg-red-500/90 text-white px-3 py-1 rounded-xl text-sm font-semibold">
                        Ganti Foto
                    </div>
                </div>

                <label class="block text-sm font-semibold text-gray-700 mt-6 mb-3">
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center hover:border-blue-400 transition-colors cursor-pointer group">
                    ...
                    <label for="image" class="cursor-pointer block">
                        ...
                        <p class="text-lg font-semibold text-gray-700 group-hover:text-blue-600">Klik untuk ganti foto (JPG, PNG, max 5MB)</p>
                        <p class="text-sm text-gray-500 mt-1">Biarkan kosong untuk tetap pakai foto lama</p>
                    </label>
                </div>
            ...
            <div>
                ...
                <select name="kategori" class="w-full px-4 py-4 border border-gray-300 rounded-2xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all shadow-sm @error('kategori') border-red-400 @enderror" required>
                    ...
                    <option value="Kegiatan" {{ old('kategori', $gallery->kategori) == 'Kegiatan' ? 'selected' : '' }}>Kegiatan Sekolah</option>
                    <option value="Prestasi" {{ old('kategori', $gallery->kategori) == 'Prestasi' ? 'selected' : '' }}>Prestasi Siswa</option>
                    <option value="Fasilitas" {{ old('kategori', $gallery->kategori) == 'Fasilitas' ? 'selected' : '' }}>Fasilitas</option>
                    <option value="Guru" {{ old('kategori', $gallery->kategori) == 'Guru' ? 'selected' : '' }}>Guru & Staf</option>
                    <option value="Siswa" {{ old('kategori', $gallery->kategori) == 'Siswa' ? 'selected' : '' }}>Siswa</option>
                    <option value="Lainnya" {{ old('kategori', $gallery->kategori) == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                </select>
                ...
                <textarea name="description" rows="5" 
                          ...
                          placeholder="Tulis deskripsi lengkap tentang foto ini...">{{ old('description', $gallery->description) }}</textarea>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-4 mt-12 pt-8 border-t border-gray-200">
            ...
            <button type="submit" 
                    class="bg-emerald-600 text-white px-10 py-4 rounded-2xl font-bold shadow-xl hover:shadow-2xl hover:bg-emerald-700 transition-all flex items-center">
